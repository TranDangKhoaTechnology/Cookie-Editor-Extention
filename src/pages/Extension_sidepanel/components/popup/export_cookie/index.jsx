...
import {formatCookies} from "../../../../../utils/cookie_format";
...
const ExportCookie = () => {
    ...
    const handleActionExport = async () => {
        ...
        if (!isHttpUrl(tab?.url)) {
            settingStore.alert = {type: "error", message: "Export cookie error"}
            settingStore.popup = "";
            return;
        }

        const cookies = await chrome.cookies.getAll({url: tab.url});
        const dataExport = formatCookies(cookies, format);
        typeFile = format === "json" ? "json" : "txt";
        ...
        if (password.length === 0) {
            ...
            const cipherText = CryptoJS.AES.encrypt(dataExport, password).toString();
