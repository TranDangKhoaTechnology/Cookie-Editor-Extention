...
const EditCookie = ({cookie}) => {
    ...
    const handleEditCookie = (param, value) => {
        if (param === "expirationDate") {
            settingStore.cookie_select.expirationDate = value ? Math.round(new Date(value).getTime() / 1000) : "";
            settingStore.cookie_select.session = !value;
    ...
    const handleActionEditCookie= async () => {
        ...
        if (!isHttpUrl(tabCurrent?.url) || !settingStore.cookie_select.name?.trim()) {
            settingStore.alert = {type: "error", message: "Edit cookie error"}
            settingStore.popup = "";
            return;
        }

        const removeUrl = getCookieUrl(cookie, tabCurrent.url);
        const dataUpdate = buildCookieSetDetails(settingStore.cookie_select, tabCurrent.url, {includeDomain: true});
