...
const AddCookie = () => {
    ...
    const handleSetCookie = async () => {
        try {
            ...
            if (!isHttpUrl(tabCurrent?.url) || !cookie.name?.trim()) {
                settingStore.alert = {type: "error", message: "Add cookie error"}
                settingStore.popup = "";
                return;
            }

            await chrome.cookies.set(buildCookieSetDetails(cookie, tabCurrent.url));
    ...
    useEffect(() => {
        if (settingStore.popup === "add_cookie") {
            chrome.tabs.query({active: true, lastFocusedWindow: true}).then(([tabCurrent]) => {
                if (isHttpUrl(tabCurrent?.url)) {
                    setCookie((current) => ({
                        ...current,
                        secure: tabCurrent.url.startsWith("https://"),
                    }));
                }
            });
        }
                                ...
                                <div className={`w-full`}>
                                    ...
                                    <div className={`w-full mb-2`}>
                                        ...
                                        <input
                                            id="path"
                                            type="text"
                                            onChange={(e) => handleAddCookie("path", e.target.value)}
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2"
                                            value={cookie?.path}
                                        />
                                    </div>
                                    ...
                                    <div className={`w-full mb-2`}>
                                        ...
                                        <select
                                            onChange={(e) => handleAddCookie("httpOnly", e.target.value === "true")}
                                            value={String(cookie?.httpOnly)}
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2">
