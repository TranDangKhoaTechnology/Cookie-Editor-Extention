...
import {parseHeaderCookies, parseNetscapeCookies, looksLikeNetscapeCookies} from "../../../../../utils/cookie_import";
...
const ImportCookie = () => {
    ...
    const handleImportCookieJson = async (dataCookies) => {
        ...
        for (let i = 0; i < cookies.length; i++) {
            await chrome.cookies.set(buildCookieSetDetails(cookies[i], url));
    ...
    const handleImportCookieString = async (cookies) => {
        const cookieArray = parseHeaderCookies(cookies);
        for (let i = 0;  i < cookieArray.length; i++) {
            await chrome.cookies.set(buildCookieSetDetails({
                ...cookieArray[i],
                expirationDate: Math.round(Date.now() / 1000) + 365 * 86400,
                secure: cookieArray[i].name.startsWith("__Secure-")
            }, url));
    ...
    const handleImportCookieNetscape = async (cookies) => {
        const cookieArray = parseNetscapeCookies(cookies.join("\n"));
        for (let i = 0; i < cookieArray.length; i++) {
            await chrome.cookies.set(buildCookieSetDetails(cookieArray[i], url));
    ...
    const handleImport = async () => {
        try {
            ...
            if (password.length > 0) {
                ...
                try {
                    const parsed = JSON.parse(dataCookies);
                    dataCookies = typeof parsed === "string" ? parsed : JSON.stringify(parsed);
                } catch {
                    // Raw encrypted exports do not need JSON unwrapping.
                }
            }
