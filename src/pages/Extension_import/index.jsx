...
import {OWNER_FACEBOOK_URL, OWNER_NAME, OWNER_WEBSITE_URL, WEBSITE_URL} from "../../config";
...
const ExtensionImport = () => {
    ...
    const handleImportCookieJson = async (dataCookies) => {
        ...
        if (!isHttpUrl(cookieInfo.domain)) {
            setMessage(extension.getLang("msg_cookie_error"));
            return;
        }

        for (let i = 0; i < cookies.length; i++) {
            await chrome.cookies.set(buildCookieSetDetails(cookies[i], cookieInfo.domain));
    ...
    useEffect(() => {
        if (uuid.length > 0) {
            (async () => {
                setIsLoading(true);
                try {
                    const response = await fetch(`${WEBSITE_URL}/api/cookie/${uuid}/info`, {
                        "method": "GET",
                        "mode": "cors",
                        "credentials": "include"
                    });
            ...
            <div className={`w-full mt-10 border-t border-gray-200 py-5`}>
                ...
                <div className={`w-full text-center`}>
                    <p className={`text-gray-800 text-sm mb-2`}>
                        ...
                        href={OWNER_FACEBOOK_URL}><span
                        className={`text-blue-600 font-bold`}>{OWNER_NAME}</span></a>
                    </p>
                    <p className={`text-gray-800 text-sm`}>
